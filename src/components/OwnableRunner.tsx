...
export default function OwnableRunner(props: {ownable: TypedOwnable}): JSX.Element {
    ...
    const load = async () => {
        ...
        const init = `init("${wasm}").then(resp => { self.postMessage({ success: true, msg: "WASM instantiated successfully" }) });`;

        // execution
        const consume_msg = `{"consume":{"amount":12}}`;
        const msg_info = `{"sender":"3MpuY7PRKN9XNpkHvohhmKAamE43QTqSWTb","funds":[]}`;
        const ownable_id = `only-for-testing`; // only relevant for logs on wasm side
        ...
        const exec = `execute_contract(${consume_msg},${msg_info},${ownable_id},${state_dump}).then(resp => { self.postMessage({ success: true, msg: "WASM executed successfully" }) });`;
        ...
        const query = `query_contract_state(${query_msg},${state_dump}).then(resp => { self.postMessage({ success: true, msg: "WASM queried successfully" }) })`;
